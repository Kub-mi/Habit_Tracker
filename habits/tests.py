...
def test_owner_can_update(jwt_user):
    client, user = jwt_user
    h = Habit.objects.create(user=user, place="дом", time="08:00", action="чай", duration_sec=60, periodicity_days=1)
...
def test_other_user_cannot_update(jwt_user, user_factory):
    ...
    h = Habit.objects.create(user=other, place="дом", time="08:00", action="чай", duration_sec=60, periodicity_days=1)
    url = reverse("habit-detail", args=[h.id])
    res = client.patch(url, {"action": "кофе"}, format="json")
    assert res.status_code in (403, 404)  # 403 из IsOwner, либо 404 если фильтруешь queryset

@pytest.mark.django_db
def test_public_list_is_open(api_client, user):
    Habit.objects.create(user=user, place="парк", time="21:00", action="прогулка",
                         duration_sec=60, periodicity_days=1, is_public=True)
